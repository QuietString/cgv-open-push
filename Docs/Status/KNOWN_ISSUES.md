...
| ISSUE-011 | High | Blocked | Kakao self-message delivers chat content but does not produce incoming-message push notifications, so it cannot by itself provide attention-getting opening alerts. | Owner reported the distinction on 2026-09-03; [Kakao's official response](https://devtalk.kakao.com/t/rest-api/142306) confirms this is intentional memo behavior. An isolated sender test exited 0, which proves API acceptance, not a device notification. | Obtain the owner's choice of a push-capable transport; do not claim that self-message delivery or a successful API response verifies push alerts. |
