...
def find_changes(previous, current):
    ...
    for operation, value in changes:
        extracted = extract_all_text_from_xml(value)
        if operation == 1:
...
def screen_main(url, cookies, headers, json_data, target_name, message_queue):
    ...
    while True:
        try:
            ...
            save_log_info(f"{target_name} response : 위치 : {location}, 유형 : {screen_type}")
            current = screen_remove_useless_tags(response)

            if previous is not None and previous != current:
                added, deleted = find_changes(previous, current)
                if added:
